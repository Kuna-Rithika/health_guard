from .qdrant_service import store_health_record
...
def analyze_health(data: AnalyzeRequest):
    ...
    # Save Session
    risk_text = str(result["report"]["risk_assessment"])
    risk_level, risk_score = parse_risk_assessment(risk_text)

    cursor.execute("""
        INSERT INTO symptom_history(
            user_id, symptoms, risk_level, risk_score, ai_summary
        )
        VALUES (%s, %s, %s, %s, %s)
    """, (
        data.user_id,
    ...
    conn.commit()
    store_health_record(data.user_id, data.symptoms, risk_text)
    conn.close()
